...
export class UpdateUserComponent {
  _topicHeader: string = 'แก้ไขข้อมูลผู้ดูแล'
  alertError: any
  userData:any = []
  constructor(
  ...
  async updateUser(result: any) {
    ...
    await setDoc(doc(firestore, "Caregivers", this.userData.employeeId), this.userData);
    this.location.back()
  }

  async querySigle(employeeId: any) {
    const data = query(collection(firestore, 'Caregivers'), where("employeeId", "==", employeeId));
